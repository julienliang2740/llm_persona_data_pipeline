...
|---|---|---|---|
| `ctext.org` (Chinese Text Project) | The standard online *Sanguozhi*, with parallel translations and a concordance | `robots.txt` retrieved and read. It names `GPTBot`, `ChatGPT-User`, `Amazonbot` and `ia_archiver` with `Disallow: /`, together with a long list of site-mirroring and bulk-download tools including `Python-urllib` and `Python`. Its `User-agent: *` group blocks only `/admin` and `/hp.pl`, and no Claude agent is named. | **Declined**, consistent with `targets/confucian/SOURCES.md`, which read the same file on 2026-09-07 and reached the same decision. The site's evident intent is to refuse automated retrieval by AI crawlers and downloaders; the absence of a rule naming a Claude agent is an omission, not a permission. Wikisource used instead. |
